...
function connect() {
    ...
    socket.on('connection success', () => {
        ...
        status = NO_USERNAME
        let message = '<font size="3"><span uk-icon=\'icon: check; ratio: 1.5\'></span> Sucessfully Connected ! </font>'
        ...
        socket.on('accepted username', usrnm => {
                ...
                let message = '<font size="3"><span uk-icon=\'icon: check; ratio: 1.5\'></span> ❤ welcome to mumble ❤ </font>' + username + '<font size="3"> !</font>'
                UIkit.notification({ message: message, pos: 'bottom-center', status: 'success' })
                    /* Reveal spinner page */
                ...
                socket.on('joined room', data => {
                    ...
                    let message = '<font size="3"><span uk-icon=\'icon: users; ratio: 1.5\'></span> You are in the room with </font>' + data.room + '<font size="3"> !</font>'
                    UIkit.notification({ message: message, pos: 'bottom-center', status: 'success' })
                        /* Go chatting! */
                    status = CHAT_ON
        ...
        socket.on('used username', () => {
            status = NO_USERNAME
            let message = '<font size="3"><span uk-icon=\'icon: warning; ratio: 1.5\'></span>  Your name is already in use ! </font>'
...
function connectionError() {
    let message =
        '<font size="3"><span uk-icon=\'icon: warning; ratio: 1.5\'></span>  It looks like you are having trouble ...</font>'
    UIkit.notification({
        message: message,
        pos: 'bottom-center',
        status: 'warning',
...
function setUsername(socket) {
    ...
    if (username == '') {
        let message = '<font size="3"><span uk-icon=\'icon: question; ratio: 1.5\'></span> Dont you have a name ? </font>'
        UIkit.notification({ message: message, pos: 'bottom-center' })
    } else if (username.length > 14) {
        let message = '<font size="3"><span uk-icon=\'icon: warning; ratio: 1.5\'></span> Your name is too long ! </font>'
        UIkit.notification({ message: message, pos: 'bottom-center' })
    } else {
...
function checkUserIsTyping(socket) {
    ...
    document.getElementById('userMessageInputField').onkeypress = () => {
        ...
        searchTimeout = setTimeout(function() {
            socket.emit('user typing something...')
        }, 250)
    }
}
